...
namespace net.vieapps.Components.Security
{
	...
	public class UserIdentity : ClaimsIdentity, ISerializable
	{
		...
		/// <param name="authenticationType">The type of authentication used</param>
		public UserIdentity(string id, string name, string sessionID, List<string> roles, List<Privilege> privileges, string authenticationType = null)
		{
			this.ID = id;
			this.Name = name;
		...
		{
			var can = (originalPrivileges == null || UserIdentity.IsEmpty(originalPrivileges.DownloadableUsers, originalPrivileges.DownloadableRoles))
				&& (parentPrivileges == null || UserIdentity.IsEmpty(parentPrivileges.DownloadableUsers, parentPrivileges.DownloadableRoles))
		...
		public static Privileges NormalizePrivileges(Privileges privileges)
		{
			...
			if (UserIdentity.IsEmpty(privileges.DownloadableRoles, privileges.DownloadableUsers))
			...
			if (UserIdentity.IsEmpty(privileges.ViewableRoles, privileges.ViewableUsers))
			...
			if (UserIdentity.IsEmpty(privileges.ContributiveRoles, privileges.ContributiveUsers))
			...
			if (UserIdentity.IsEmpty(privileges.EditableRoles, privileges.EditableUsers))
			...
			if (UserIdentity.IsEmpty(privileges.ModerateRoles, privileges.ModerateUsers))
			...
			if (UserIdentity.IsEmpty(privileges.AdministrativeRoles, privileges.AdministrativeUsers))
			...
			if (UserIdentity.IsEmpty(permissions.DownloadableRoles, permissions.DownloadableUsers)
				&& UserIdentity.IsEmpty(permissions.ViewableRoles, permissions.ViewableUsers)
				&& UserIdentity.IsEmpty(permissions.ContributiveRoles, permissions.ContributiveUsers)
				&& UserIdentity.IsEmpty(permissions.EditableRoles, permissions.EditableUsers)
				&& UserIdentity.IsEmpty(permissions.ModerateRoles, permissions.ModerateUsers)
				&& UserIdentity.IsEmpty(permissions.AdministrativeRoles, permissions.AdministrativeUsers))
				permissions = null;
		...
		public static Privileges CombinePrivileges(Privileges originalPrivileges, Privileges parentPrivileges)
		{
			...
			if (originalPrivileges != null && UserIdentity.IsNotEmpty(originalPrivileges.DownloadableRoles, originalPrivileges.DownloadableUsers))
			{
			...
			if (originalPrivileges != null && UserIdentity.IsNotEmpty(originalPrivileges.ViewableRoles, originalPrivileges.ViewableUsers))
			{
			...
			if (originalPrivileges != null && UserIdentity.IsNotEmpty(originalPrivileges.ContributiveRoles, originalPrivileges.ContributiveUsers))
			{
			...
			if (originalPrivileges != null && UserIdentity.IsNotEmpty(originalPrivileges.EditableRoles, originalPrivileges.EditableUsers))
			{
			...
			if (originalPrivileges != null && UserIdentity.IsNotEmpty(originalPrivileges.ModerateRoles, originalPrivileges.ModerateUsers))
			{
			...
			if (originalPrivileges != null && UserIdentity.IsNotEmpty(originalPrivileges.AdministrativeRoles, originalPrivileges.AdministrativeUsers))
			{
		...
		public static string GetAccessToken(UserIdentity userIdentity, BigInteger eccKey)
		{
			...
			return UserIdentity.GetAccessToken(userIdentity.ID, userIdentity.SessionID, (userIdentity.Roles ?? new List<string>()).Concat(roles.ToList()), userIdentity.Privileges, eccKey);
		}
		...
		{
			var accessToken = UserIdentity.GetAccessToken(userIdentity, eccKey);
			return UserIdentity.GetJSONWebToken(userIdentity.ID, userIdentity.SessionID, encryptionKey, shareKey, (token) =>
			{
		...
		public static Tuple<UserIdentity, string> ParsePassportToken(string jwtoken, string encryptionKey, string shareKey, BigInteger eccKey, Func<string, string> getUserName = null)
		{
			var accessToken = "";
			var deviceID = "";
			var info = UserIdentity.ParseJSONWebToken(jwtoken, encryptionKey, shareKey, (token) =>
			{
			...
			var userIdentity = UserIdentity.ParseAccessToken(accessToken, eccKey, getUserName);
	...
	public static class UserIdentityExtentions
	{
		...
		{
			return UserIdentity.GetAccessToken(userIdentity, eccKey);
		}
		...
		{
			return UserIdentity.ParseAccessToken(accessToken, eccKey, getUserName);
		}
		...
		/// <param name="onPreCompleted">The action to run before the parsing process is compeleted</param>
		/// <returns>The string that presents a JSON Web Token</returns>
		public static string GetJSONWebToken(this UserIdentity userIdentity, string encryptionKey, string shareKey, Action<JObject> onPreCompleted = null)
		{
			return UserIdentity.GetJSONWebToken(userIdentity.ID, userIdentity.SessionID, encryptionKey, shareKey, onPreCompleted);
		}
		...
		{
			return UserIdentity.ParseJSONWebToken(jwtoken, encryptionKey, shareKey, onPreCompleted);
		}
		...
		{
			return UserIdentity.GetPassportToken(userIdentity, deviceID, encryptionKey, shareKey, eccKey);
		}
		...
		{
			return UserIdentity.ParsePassportToken(jwtoken, encryptionKey, shareKey, eccKey, getUserName);
		}
	}
}